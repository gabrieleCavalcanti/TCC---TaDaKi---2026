...
export class PessoaController {
  ...
  selecionaTodos = async (req: Request, res: Response) => {
    try {
      ...
      if ((id && nome) || (id && tipo) || (nome && tipo)) {
        paramDup =
          "Mais de um parâmetro informado, prioridade: ID > NOME > TIPO";
      }
  ...
  criar = async (req: Request, res: Response) => {
    try {
      ...
      if (!infoExtra.password || typeof infoExtra.password !== "string") {
        return res.status(400).json({
      ...
      if (tipoFormatado === "ORGANIZACAO") {
        ...
        if (cnpj && !/^[A-Z0-9]{14}$/.test(cnpj.toUpperCase())) {
          return res.status(400).json({
            message: "CNPJ deve conter 14 caracteres, entre letras e números",
          });
        }
      ...
      const novo = await this._service.criar(nome, tipoFormatado, infoExtra);
    ...
    try {
      ...
      if (
        ...
        if (cpf && cnpj) {
          return res
            .status(400)
            .json({ message: "Informe apenas CPF ou CNPJ" });
        }
        ...
        if (!infoExtra.id_area_atuacao) {
          return res
            .status(400)
            .json({ message: "Área de atuação é obrigatória" });
        }
      }
